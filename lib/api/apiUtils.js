...
apiUtils.dataParsoidContentType = function(env, contentVersion) {
	return 'application/json; charset=utf-8; profile="https://www.mediawiki.org/wiki/Specs/data-parsoid/' + (contentVersion || env.contentVersion) + '"';
};
...
apiUtils.dataMwContentType = function(env, contentVersion) {
	return 'application/json; charset=utf-8; profile="https://www.mediawiki.org/wiki/Specs/data-mw/' + (contentVersion || env.contentVersion) + '"';
};
...
/**
 * Downgrade content from 999.x to 1.x.
 *
 * @param {MWParserEnvironment} env
 * @param {Object} revision
 * @param {Response} res
 ...
 */
apiUtils.downgrade999to1 = function(env, revision, res, contentmodel) {
	...
	if (meta) { meta.setAttribute('content', env.contentVersion); }
...
apiUtils.updateRedLinks = Promise.async(function *(env, revision, res, contentmodel) {
	...
	apiUtils.wt2htmlRes(env, res, html, pb, contentmodel, DU.findHttpEquivHeaders(doc), env.originalVersion);
});
...
apiUtils.languageConversion = function(env, revision, res, contentmodel, targetVariant, sourceVariant) {
	...
	if (env.langConverterEnabled()) {
		...
		const ensureHeader = (h) => {
			// XXX should be case-insensitive match in querySelector, but
			// domino doesn't (yet) support this.
			// https://github.com/fgnass/domino/issues/128
			let el = doc.querySelector(`meta[http-equiv="${h}"]`);
			if (!el) {
...
apiUtils.wt2htmlRes = function(env, res, html, pb, contentmodel, headers, contentVersion) {
	if (pb) {
		...
		if (semver.satisfies(env.contentVersion, '^999.0.0')) {
			response['data-mw'] = {
